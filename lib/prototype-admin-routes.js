...
const password_temp = process.env.PASSWORD_TEMP
const password_confluence = process.env.PASSWORD_CONFLUENCE
...
router.post('/password', function (req, res) {
  ...
  else if (submittedPassword === password_temp) {
    res.cookie('authentication', encryptPassword(password_temp), {
  ...
  else if (submittedPassword === password_confluence) {
    res.cookie('authentication', encryptPassword(password_confluence), {
      maxAge: 1000 * 60 * 60 * 24, // 30 days
      sameSite: 'None', // Allows GET and POST requests from other domains
      httpOnly: true,
      secure: true
