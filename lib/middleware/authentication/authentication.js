...
function authentication () {
  ...
  const password_temp = encryptPassword(process.env.PASSWORD_TEMP)
  const password_confluence = encryptPassword(process.env.PASSWORD_CONFLUENCE)
  ...
  return function authentication (req, res, next) {
    if (allowedPathsWhenUnauthenticated.includes(req.path)) {
      ...
      next()
    } else if (isAuthenticated(password_temp, req)) {
      next()
    } else if (isAuthenticated(password_confluence, req)) {
      next()
    } else {
